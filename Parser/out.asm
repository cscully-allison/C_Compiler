...
add $t3, $t0, $t4             
li.s $f0 f0.0                 #STORE   faddr IR38   None   fconst 0.0  
...
add $t6, $t0, $t7             
lwc1 $f1 ($t5)                #LOAD   ftemp FR2   None   faddr IR58  
lwc1 $f2 ($t6)                #LOAD   ftemp FR3   None   faddr IR64  
mult $f1, $f2                 #MULT   ftemp FR1   ftemp FR2   ftemp FR3  
mflo $t0                      
lw $t7 8($sp)                 #LOAD   temp IR68   None   local 8  
li $t8, 1                     
add $t7, $t7, $t8             #ADD   temp IR68   temp IR68   const 1  
sw $t7 8($sp)                 #STORE   local 8   None   temp IR68  
j L5                          #JUMP   label L5   None   None  
L6:                           #LABEL   label L6   None   None  
lw $t7 4($sp)                 #LOAD   temp IR69   None   local 4  
li $t8, 1                     
add $t7, $t7, $t8             #ADD   temp IR69   temp IR69   const 1  
sw $t7 4($sp)                 #STORE   local 4   None   temp IR69  
j L7                          #JUMP   label L7   None   None  
L8:                           #LABEL   label L8   None   None  
lw $t7 0($sp)                 #LOAD   temp IR70   None   local 0  
li $t8, 1                     
add $t7, $t7, $t8             #ADD   temp IR70   temp IR70   const 1  
sw $t7 0($sp)                 #STORE   local 0   None   temp IR70  
j L9                          #JUMP   label L9   None   None  
L10:                          #LABEL   label L10   None   None  
li $t7 0                      #LOAD   return   None   const 0  
